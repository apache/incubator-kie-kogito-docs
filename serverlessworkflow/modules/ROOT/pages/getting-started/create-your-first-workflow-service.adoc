...
and create a `Hello World` application, which includes the following procedures:
...
For more information about the tooling and the required dependencies, see xref:getting-started/getting-familiar-with-our-tooling.adoc[Getting familiar with Serverless Workflow tooling].
...
* `{kogito_sw_gav}`: Adds support for serverless workflows.
...
For more information about Knative CLI, see xref:tooling/kn-plugin-workflow-overview.adoc[Serverless Workflow plug-in for Knative CLI].
--
====
