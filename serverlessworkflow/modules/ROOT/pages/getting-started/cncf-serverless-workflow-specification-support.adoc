...
Kogito supports events of the Workflow model as defined in the link:{spec_doc_url}#Event-Definition[Serverless Workflow specification definition]. However, `correlation` events has limited support. 

For more information about the events feature, see xref:eventing/event-correlation-with-workflows.adoc[Event correlation with Serverless Workflows].
