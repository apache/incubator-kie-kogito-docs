...
The link:{spec_doc_url}#event-state[Event State] of the workflow states feature should not be used as starting state if exclusive property is set to `false`. In case is specified that way it will behave as if exclusive was set to `true`. 
