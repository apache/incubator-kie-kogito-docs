...
For event state the `exclusive` property is not support if se to `false`, therefore the timeout is not supported for the event state when starting a workflow.
