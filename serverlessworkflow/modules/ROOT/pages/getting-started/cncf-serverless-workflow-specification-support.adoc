...
Kogito supports events of the Workflow model as defined in the link:{spec_doc_url}#Event-Definition[Serverless Workflow specification definition], except:

* `resultEventRef` property in link:{spec_doc_url}#eventref-definition[EventRefDefintion] is not implemented and, if specified, will be ignored. Same functionality can be achieved by using xref:eventing/working-with-callbacks.adoc[Callback] state.
