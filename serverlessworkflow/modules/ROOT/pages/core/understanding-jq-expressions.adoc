...
This document describes the usage of jq expressions in funtions, switch state conditions, action function arguments, data filtering and event publishing. 
...
In previous example, a Cloud Event is published when the state transitioned. Its `data` field looks like 
...
`$SECRET` always return a value of type string. If you want to use it into a comparison with a value that is not a string, you need to perform the conversion explicitly. In next example, `retries` property is a number, so we need to convert the property value accordingly by calling `tonumber` builtin jq function. 
