jq expressions in Serverless Workflow
=====================================
:compat-mode!:
// Metadata:
...
This document describes the examples of using jq expressions in switch state conditions, action function arguments, and data filtering.
...
A condition in a switch state is an expression, which returns a boolean value when evaluated against the data model. If a condition with a state transition returns true, then the workflow must follow the defined path.
...
[NOTE]
====
The Serverless Workflow Specification requires all the expressions to be embedded within `${… }`. However, in Serverless Workflow it is figured whether or not a string is an expression. Therefore, you can save characters and skip `${` in the beginning and `}` in the end. In case of portablity, you must embed the expressions within `${… }`.
====
...
In the previous example, the string evaluation results a JSON object, which is passed in an OpenAPI request.

[[ref-example-jq-expression-data-filtering]]
== Example of jq expressions in data filtering

In Serverless Workflow Specification, the data filtering mechanism is used to select the data, which must be part of the workflow data model.

The data filtering mechanism in Serverless Workflow Specification, include:
...
For example, you can see link:{kogito_sw_examples_url}/serverless-workflow-expression-quarkus[`serverless-workflow-expression-quarkus`], in which actions and state data filters are used.
...
In case the previous example of action data filter is not added in the `serverless-workflow-expression-quarkus`, then the JSON object returned by the function is merged into the workflow model. The previous action data filter contains the following properties:
...
The original `numbers` array is not returned as a result of the workflow execution, therefore the final stage consists of a state data filter defining the content of the output model. The output model is defined as a JSON object, containing a `result` property. The value of `result` property is the maximum number that is calculated by the previous state and stored in the `number` property. 

In the previous example, the output model is received by the `input` property, which means that the output model is changed before the state is executed. As a final result, the output model consists of a `result` property, containing the maximum value of `x`.
