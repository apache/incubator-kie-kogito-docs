...
The string after the : is optional and is used to indicate the log level. Possible values are TRACE, DEBUG, INFO, WARN and ERROR. If not present, INFO is assumed. 
...
----
`message` argument can be an jq expression or a jq string using interpolation, as in the example above
