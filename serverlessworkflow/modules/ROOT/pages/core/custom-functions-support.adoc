...
By default, {product_name} sends the payload of a Knative function as a regular JSON object. However, you can tell {product_name} to send the payload as a CloudEvent. In that case, {product_name} will check if the CloudEvent has all mandatory attributes set.

To tell {product_name} you want to send the payload as a CloudEvent, you must define your function like:
...
[WARNING]
====
You can't send a CloudEvent if you don't set the `asCloudEvent` attribute to `true`. If you do so, {product_name} will throw an error.
====
...
Given you want to send the following JSON object as the payload:
...
Given you want to send the following CloudEvent as the payload:
...
You must declare a `functionRef` like the following: (Don't forget to set `asCloudEvent` as `true` in function metadata. See <<sending-cloudevents>>.)
