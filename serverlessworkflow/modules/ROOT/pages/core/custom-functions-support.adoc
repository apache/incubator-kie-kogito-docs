...
You can add your own custom types by using Kogito add-on mechanism. As predefined custom types like `sysout` or `java`, the custom type identifier is the prefix of the operation field of the function definition. 

Kogito add-ons relies on link:{quarkus_guides_base_url}/writing-extensions[Quarkus extensions] mechanism. And add-on consists of at least two Maven projects:
...
----
`operation` starts with `rpc`, which is the custom type identifier, and continues with `division`, which denotes the operation that is going to be executed in the legacy server

In order for this function definition to be recognized, a Kogito addon that defines the `rpc` custom type needs to be developed. It consists of a  link:https://github.com/kiegroup/kogito-examples/tree/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-deployment[deployment project] and a link:https://github.com/kiegroup/kogito-examples/tree/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc[runtime project]

The deployment project is responsible for extending link:https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-deployment/src/main/java/org/kie/kogito/examples/sw/services/RPCCustomTypeHandler.java[`WorkItemTypeHandler`] and setup the `WorkItemNodeFactory`
