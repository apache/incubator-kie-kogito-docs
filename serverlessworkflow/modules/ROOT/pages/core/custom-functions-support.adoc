...
Your method interface signature should mimic the arguments passed by the flow. 

For example, if you invoke the function using one argument: 
...
Your method signature should be, assuming `number` model variable is an integer 
...
As special case, if no argument is provided in the workflow definition, the signature of the Java method might include a JsonNode parameter. This means that the Java method is expecting the whole flow model as input.

For example, using this no argument function reference 
...
if the method signature has a `JsonNode` parameters as in the following example, the whole model is passed when the method call is performed. 
...
If your methods returns a `JsonNode`, the content of that node is merged into the Workflow model (you can use an action data filter to control what is merged)

The same occurs if your methods returns any Java `Object` descendant that is not a primitive wrapper, the Java Object is recursively converted to a JSON one and the result is merged into the Workflow model (you can use an action data filter to control what is merged) 

If your methods returns a primitive type or their corresponding wrapper object (int, boolean, long...), then the primitive value is added to the workflow model with the name `response` (you can change that name using an action data filter) 

If you method returns Java collections, it is converted to a JSON array and added to the workflow model with the name `response` (you can change that name using an action data filter)

=== Function accessing Kogito context

If you need access to process contextual information (for example, kogito process instance id) inside your java service, you can add a `KogitoProcessContext` parameter to the method signature

Therefore, if you need to do so, you can update signature of methods from previous sections
