...
You can add your our custom types by using Kogito add-on mechanism. As predefined custom types like `sysout` or `java`, the custom type identifier is the prefix of the operation field of the function definition. 

Kogito add-ons relies on link:https://es.quarkus.io/guides/writing-extensions[Quarkus extensions] mechanism. And add-on consist of at least two Maven projects:
- deployment, which is responsible for generating code required for the extension to work.
- runtime, which include the non generated classes that are required for the extension to work. 

In the case of a serverless workflow custom type:

- Deployment project is expected to configure the work item handler that will be used during runtime to perform the logic associated to the custom type. It must contain a Java class that inherits from `WorkItemTypeHandler`. Its responsibilities are to indicate the custom type identifier (the operation prefix, as indicated earlier) and to setup the `WorkItemNodeFactory` instance passed as parameter of `fillWorkItemHandler` method. That instance is included in the Kogito process definition for that Workflow. As part of this setup, you must indicate the name of the  `WorkItemNodeFactory`. You might also provide any relevant metadata for that handler, if needed. 
- Runtime project consists of a `WorkflowWorkItemHandler` implementation, which name must match the one provided to `WorkItemNodeFactory` during the deployment phase, and a `WorkItemHandlerConfig` bean that register that handler with that name. 
 When a Serverless workflow function is called, Kogito identifies the proper `WorkflowWorkItemHandler` instance to be used for that function type (using the handler name associated to that type by the deployment project) and then invokes `internalExecute` method. The `Map` parameter contains the function arguments defined in the Workflow and the `WorkItem` parameter contains the  metadata information added to the handler by the deployment project. Hence, `executeWorkItem` implementation has access to all the information needed to perform the computational logic intended for that custom type. 


=== Custom function type example. 

Lets assume we want to interact, from a workflow file, with a legacy RPC server as the one defined link:https://github.com/kiegroup/kogito-examples/tree/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-server[this project]. This legacy server support four simple arithmetic operations: add, minus, multiply an divide, that can be invoked using a custom RPC protocol. Since this a non common protocol, workflow cannot handle them by using any of the predefined Serveless workflow function types. The available options are to use a Java Service, which invokes a Java class that knows how to interact with the server; or define a custom type that knows how to interact with the service. 

Using  the latter approach, users can write a link:https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-type-example/src/main/resources/customType.sw.json[workflow file] defining this function. 
...
----
`operation` starts with `rpc`, which is the custom type type identifier, and continues with `division`, which denotes the operation that is going to be executed in the legacy server

In order this function definition to be recognized, a Kogito addon that defines the `rpc` custom type needs to be developed. It consist of a  link:https://github.com/kiegroup/kogito-examples/tree/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-deployment[deployment project] and a link:https://github.com/kiegroup/kogito-examples/tree/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc[runtime project]

Deployment project is responsible to extend link:https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-deployment/src/main/java/org/kie/kogito/examples/sw/services/RPCCustomTypeHandler.java[`WorkItemTypeHandler`] and setup the `WorkItemNodeFactory`
...
----
This setups the name of the `KogitoWorkItemHandler` and adds a metadata key that extracts the name of the operation to performed in the legacy server from the Serverless workflow function definition

Runtime project contains the link:https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc/src/main/java/org/kie/kogito/examples/sw/custom/RPCCustomWorkItemHandler.java[KogitoWorkItemHandler] and the https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc/src/main/java/org/kie/kogito/examples/sw/custom/RPCCustomWorkItemHandlerConfig.java[WorkItemHandlerConfig] implementations. 


As expected, RPCCustomWorkItemHandler implements `internalExecute` method
[source, java]
----
 @Override
...
The implementation invokes link:https://github.com/kiegroup/kogito-examples/blob/stable/serverless-workflow-examples/serverless-workflow-custom-type/serverless-workflow-custom-rpc-server/src/main/java/org/kie/kogito/examples/sw/custom/CalculatorClient.java#L45-L67[CalculatorClient.invokeOperation], a java static method that knows how to interact with the legacy service. The operation parameter is obtained from WorkItem metadata. Dividend and divisor parameters are obtained from the Map parameter, which contains the function arguments as defined in the workflow file.
...
RPCCustomWorkItemHandlerConfig is a bean which registers the handler name
