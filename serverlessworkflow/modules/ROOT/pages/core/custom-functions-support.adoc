...
To call the function shown in the previous example within your workflow service, you can extract the required JSON value using xref:core/understanding-jq-expressions.adoc[`jq` expression] or pass the JSON value without using arguments.
...
In the previous example, the `JsonNode` that the `java` function returns is the new payload in the workflow execution.
