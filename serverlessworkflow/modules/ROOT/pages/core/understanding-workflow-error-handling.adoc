...
Kogito checks whether the message of the Java exception matches the value provided in the error `code`. The validation is performed using a regex matcher in case-insensitive manner, and will be successful if a partial match is found.
...
|`Unknown error, status code 400`|`Status code`
|`Unknown error, status code 400`|`(.*)status code 4[0-9]\{2\}`
|=======================

==== HTTP status code

Define the mapping as a HTTP status code returned by the invoked service.

For exceptions of type `javax.ws.rs.WebApplicationException`, Kogito checks the defined error `code` against the HTTP code from the underlying response object.  Such exceptions can be encountered when using REST or OpenAPI service invocations. The error code can be defined either as the HTTP code itself, or as colon separated string where the error code is in the second part.
