...
**Example**: `"code": "java.lang.RuntimeException"`

==== Error message

Define the mapping as a (partial) match of the error message.

Kogito checks whether the message of the Java exception matches the value provided in the error `code`. The validation is performed after converting all Strings to lowercase, and will be successful if a partial match is found. This implementation also supports the usage of link:{java_regex_url}[regex] patterns.

**Examples**: `"code": "Unknown error"`, `"code": "(.*)status code 4[0-9]\{2\}"`

==== HTTP status code

Define the mapping as a HTTP status code returned by the invoked service.

For exceptions of type `javax.ws.rs.WebApplicationException`, Kogito checks the defined error `code` against the HTTP code from the underlying response object. The error code can be defined either as the HTTP code itself, or as colon separated string where the error code is in the second part.

**Examples**: `"code": "500"`, `"code": "HTTP:404"`
