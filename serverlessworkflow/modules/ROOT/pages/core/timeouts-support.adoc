...
Regardless its application scope (workflow or state), timeouts can not be defined as a specific point in time, but it should be an amount of time, a duration, which is considered to start when the referred scope becomes active. Timeouts uses link:https://en.wikipedia.org/wiki/ISO_8601[`ISO 8601` data and time standard] to specify a duration of time.
...
You can set up the maximum amount of time a workflow might be running before being cancelled. This is configured in the header section of the flow, by using link:{spec_doc_url}#workflowexectimeout-definition[WorkflowExecTimeout definition]. Only `duration` property is currently implemented. 
Once cancelled, the workflow is considered finished and won't be accessible though a GET request anymore. So it behaves as interrupt was `true` by default. 
