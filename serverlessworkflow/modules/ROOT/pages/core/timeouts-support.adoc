...
The `event` state is used to wait for one or more events to be received by the workflow and then continue the execution if it is not a starting state.

[NOTE]
====
Event state is not supported as starting state if `exclusive` flag is set to `false`, therefore timeout is not supported in this case.
====
...
If this time is exceeded and the events were not received, the workflow moves to the state defined in the transition property or ends the workflow instance without performing any actions in case of an end state.
...
If none of the configured events arrive before the timeout is overdue, the workflow execution moves to the next state defined in the corresponding `transition` property, skipping the events that were not received in time.

If one of the events arrives before the timeout is overdue, the workflow then moves to the state defined in `transition`, executing the corresponding event that has arrived in the state definition.
