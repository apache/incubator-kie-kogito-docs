...
[NOTE]
====
The Job Service link:{jobs_service_image_url}[image] is equipped with the PostgreSQL and InMemory options that can be
switched using the `JOBS_SERVICE_PERSISTENCE`. If not set, it defaults to the `inmemory` option.
More details about the Job Service Container image can be found link:{jobs_service_image_usage_url}(here).
====
[NOTE]
====
The job service link:{jobs_service_image_url}[image] is equipped with the PostgreSQL and InMemory options that can be switched using the `JOBS_SERVICE_PERSISTENCE`. If not set, it defaults to the `inmemory` option. For more information about the job service, the container image can be found link:{jobs_service_image_usage_url}(here).
====
