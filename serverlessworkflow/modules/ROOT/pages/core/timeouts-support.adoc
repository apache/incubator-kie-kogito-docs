...
=== Job Service Leader election feature

Currently, job service works in a single instance manner where there should be just one running and active instance of the service.

To avoid issues when deploying the service into the cloud where it is common to eventually have more than one instance deployed, job service supports a leader instance election process, where an instance once became the leader, activates the external communication to receive and schedule jobs internally.
...
When an instance that is the leader for any issue stays irresponsive or is shut down, one of the other running instances becomes the leader.
...
There is no need for any configuration to support this feature, the only requirement is to have the supported database with the data schema
