...
.Excerpt of the Callback State declaration handling with the waitEvent
[code,json]
----
{
      "name": "waitForEvent",
...
.Excerpt of the Java method that consumes the event "resume"
[code,java]
----
...
The cloud event published by the listener modifies the original value of the `move` field and publishes it as the `result` field. Moreover, it contains a header attribute, named `kogitoprocrefid`, holding the process instance id of the workflow that published the event.
...
.Excerpt of the event consumer in the callback example
[source,java]
----
    @Incoming("in-resume")
    @Outgoing("out-wait")
