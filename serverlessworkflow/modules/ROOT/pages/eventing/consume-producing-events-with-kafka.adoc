...
:overflow_annotation_url: https://smallrye.io/smallrye-reactive-messaging/2.3/apidocs/org/eclipse/microprofile/reactive/messaging/OnOverflow.html
...
Kogito will annotate generated smallrye `emitters` for a particular channel using the information provided by a property of the form `kogito.addon.messaging.emitter.<channel_name>.overflow-strategy`, which possible values are `BUFFER`,`NONE`,`UNBOUNDED`,`FAIL` and `DROP`. If the value of strategy is `BUFFER`, then you might specify the buffer size by using property of the form `kogito.addon.messaging.emitter.<channel_name>.buffer-size` 

Therefore, for `wait` channel, in previous example, if we want to buffer as much as 100 events we will add these two properties
...
If all your channels are using the same strategy and this strategy is different from `BUFFER` one (you can change buffer size globally by using smallrye property `mp.messaging.emitter.default-buffer-size`), you can set up it by using `kogito.addon.messaging.emitter.overflow-strategy=NONE|UNBOUNDED|FAIL|DROP`
