...
This document describes how you can configure Serverless Workflow to interact with link:{kafka_doc_url}[Apache Kafka]. Apache Kafka uses an abstraction called topic to classify events. The events are published to a topic, and events are consumed from a topic. 
...
To enable Serverless Workflow to use Apache Kafka Smallrye connector, add the following dependency to the `pom.xml` file of your project if using Maven:
...
The messaging capabilities are included in the Quarkus Serverless Workflow extension, even though the messaging capabilities are optional. This means you do not need to explicitly add the messaging add-on dependency when using Serverless Workflow. However, you need to add the Kafka Quarkus Smallrye connector dependency to indicate that you are using Apache Kafka.
...
* Define a channel for certain CloudEvent types. Allow the non-mapped CloudEvent types to use the default incoming or outgoing channel.
...
The following properties are mandatory that you need to set for each channel:
...
[TIP]
====
`topic` is another relevant but optional property that needs to be set for a channel. The `topic` property must contain the Apache Kafka topic name to be used for a channel. If `topic` property is not set, then the channel name is used as topic name. 
====
...
This section describes the examples of channel mapping by reviewing the properties that are configured in the previous section. 
...
As mentioned before, Kafka topic names matches the CloudEvent types. However, to use the default channel functionality, `kogito_incoming_stream` and `kogito_outgoing_stream` are used as channel names. If there is no specific channel name for the CloudEvent type, then the default channels are used. Also, the default channels need to be mapped to the desired topic name using the `topic` property.
