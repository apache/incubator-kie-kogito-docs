...
The `serverless-workflow-callback-events-over-http-quarkus` application is initiated with the following request:
[code,json]
----
{
  "message": "Hello"
...
After that, a Callback state is declared, and it waits for a CloudEvent with the `wait` type. Following is an example of declaring a Callback state that handles the `wait` type CloudEvent:
