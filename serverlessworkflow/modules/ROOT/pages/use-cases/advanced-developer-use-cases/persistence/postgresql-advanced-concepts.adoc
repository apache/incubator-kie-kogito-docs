...
[NOTE]
====
Regular backup procedures are not performed by the workflows runtime persistence, and thus, they must be provided by the installation owner.
====

== `process_instances`
This table stores the workflows snapshots. These snapshots are stored in binary format, and by default, are maintained in the database as long as the workflow instance is active. When a workflow instance finalizes it execution, the corresponding snapshot is removed from the database for optimization purposes.
...
If the link:{flyway_url}[Flyway] database schema generation was enabled for the current workflow project, this table contains the information about the schema versions and the corresponding updates. And is completely managed by that framework.
