...
The SonataFlow workflows runtime persistence is the mechanism to ensure that your workflow instances remains consistent in the context of an error or a runtime restart. For example, a pod restart, a programmed maintenance shutdown, etc.

Every workflow instance requires some status information and data to execute, this information is automatically managed by the workflows runtime, and is persisted at different moments of the workflow execution.

For example, when a workflow instance reaches a state that needs to wait for an event, the engine takes a snapshot of the most relevant information, stores it in the database, and pause that instance execution.
