= Saga orchestration example in Serverless Workflow
...
The Saga design pattern manages data consistency across services that are available in distributed transaction scenarios. For more information about Saga pattern, see the initial link:https://www.cs.cornell.edu/andru/cs711/2002fa/reading/sagas.pdf[publication]. 

In a microservice architecture, you can define a service as microservice, which is responsible to perform actions related to the business domain. 
...
== Example of Saga pattern in Serverless Workflow

To understand the implementation of Saga pattern in Serverless Workflow, you can use the link:{kogito_sw_examples_url}/serverless-workflow-saga-quarkus[`serverless-workflow-saga-quarkus`] example application in GitHub repository. 
...
2. *Stock reservation*: The selected item in the order is removed from the reserved stock.
...
In the `serverless-workflow-saga-quarkus` example application, a link:{spec_doc_url}[Serverless Workflow] is used that implements the Saga pattern, in which all the steps and compensation actions are defined. Also, the Serverless Workflow plays the role of Saga Executor Coordinator (SEC), which orchestrates the calls to the microservices in the Saga pattern.
...
In the previous example figure of workflow, the calls are orchestrated to the microservices, each microservice can throw possible errors, and compensations for each step are defined, that are executed once an error appears during the workflow execution.

To define the interactions among microservices in the Saga pattern using Serverless Workflow specification, you can use link:{spec_doc_url}#workflow-states[workflow states] with link:{spec_doc_url}#Transitions[transitions]. 

In Serverless Workflow specification, each workflow state represents a step to be completed in the Saga pattern. Also, an action associated with the workflow state represents how a microservice is invoked to execute a given step.
...
+
--
In Serverless Workflow, errors are identified by a name and can be associated with a workflow state. For example, a `process payment failed` error is associated with the `processPayment` state. 
...
The function that are throwing errors can be any type of functions, such as REST, OpenAPI, or gRPC. For information about error handling, see xref:core/understanding-workflow-error-handling.adoc[Error handling in Serverless Workflow].
--

The Serverless Workflow engine controls the execution of the flow and keeps the track of the steps that need to be compensated. Also, the engine ensures that compensated states are executed in reverse order of each completed step. 

The engine is a stateful, allowing Saga to contain wait states, such as callbacks. After each wait state, the workflow is persisted and can continue once it recevies a request or event.
...
== Examples of running and testing the Saga pattern in Serverless Workflow

You can use the following examples as a reference to run and test the Saga pattern in Serverless Workflow:
...
+
--
To test the workflow, an optional `failService` attribute is introduced, indicating which microservice must respond with an error. In the following example, the `ShippingService` state throws an error, which breaks the workflow execution and triggers the compensation actions:
