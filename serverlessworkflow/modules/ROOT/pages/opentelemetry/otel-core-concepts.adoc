...
Each workflow execution generates a flat trace structure where all workflow state spans are siblings under the HTTP request span:

```
HTTP Request Span (Parent)
├── Start (infrastructure node)
├── ChooseOnLanguage (workflow state)
├── GreetPerson (workflow state)
├── ExecuteSubflow (workflow state)
├── SubflowState1 (subflow state - flat sibling)
├── SubflowState2 (subflow state - flat sibling)
└── End (infrastructure node)
```

This flat hierarchy approach offers several benefits:

* **Simplified Analysis**: All workflow states are immediately visible at the same level
* **Consistent Structure**: Both main workflow and subflow spans follow the same flat pattern
* **Reduced Complexity**: No deeply nested span trees that can be difficult to navigate
* **Clear Timeline**: Easy to visualize the chronological order of state execution
...
|Name of the current workflow state being executed
