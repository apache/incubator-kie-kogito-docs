...
You can configure the Service Discovery with two **build time** properties, which are:

* `quarkus.kogito.sw.discovery.enabled`: Enable the service discovery. Defaults to _true_.
* `quarkus.kogito.kubernetes-client.okhttp.static.logger`: Weather or not to print the `http` communication between the +
client and the Kubernetes API. Defaults to _false_.
...
[NOTE]
====
Note that, the application property does not matter. The Service Discovery only cares about the application property value.
====
