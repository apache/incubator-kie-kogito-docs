...
For **OpenShift** specific resources, **Deployment Config** or **Routes**, you will be required to add the following dependency in your Application's classpath:
...
It can be added with link:{quarkus_cli_url}[Quarkus CLI]:
[source,shell]
----
quarkus extension add quarkus-openshift-client
----
====
...
However, the okhttp communication interceptor, which logs the communication between the application and the Kubernetes API +
...
2022-08-23 12:21:05,420 DEBUG [org.kie.kog.add.qua.k8s.par.KubeURI] (main)  KubeURI successfully parsed: knative:v1/Service/serverless-workflow-greeting-quarkus/greeting-quarkus-cli
