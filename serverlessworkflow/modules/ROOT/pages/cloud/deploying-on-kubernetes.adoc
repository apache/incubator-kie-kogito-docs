...
quarkus.container-image.group=spolti <5>
...
quarkus build -- -Pcontainer -DskipTests \
  -Dquarkus.container-image.push \
  -Dquarkus.container-image.registry=quay.io \
  -Dquarkus.container-image.group=spolti \
  -Dquarkus.container-image.tag=1.0-SNAPSHOT
----

Note that the maven profile activated is named as `container`, which provide the needed system properties to build the
