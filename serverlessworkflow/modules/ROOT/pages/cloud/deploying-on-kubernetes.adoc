...
:kubectl_prereq: `kubectl` command-line tool is installed.
...
This document describes how to deploy your workflow application using a local Kubernetes cluster along with a procedure
to run the Knative platform.

// shared pre req
include::common/_prerequisites.adoc[]

Before proceeding, make sure that you have access to the Kubernetes cluster with Knative available.

[[proc-verify-knative-kubernetes]]
== Verifying Knative on Kubernetes
...
For more information on how to figure ou if Knative is installed please refer to this link:{verify_knative_url}[link].
...
Once Knative is ready, you can initiate the process of deploying your workflow application on Kubernetes.
...
[IMPORTANT]
====
By default, Kubernetes does not have any registry installed by default. You can check with the administrator if a
private registry is available to use, otherwise, you can publish the Application Container image on link:{quay_io_url}[Quay.io],
or on any other registry of your preference.
...
Preparing the `namespace` to deploy the {context} Application:
...
[IMPORTANT]
====
For this tutorial we will be using the `default-domain` provided by Knative that configures *Knative Serving* to use
