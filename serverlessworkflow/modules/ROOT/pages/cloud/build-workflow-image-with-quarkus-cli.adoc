...
- Docker - recommended to use always the latest version.
    * It's not a problem if you don't have Docker installed or can't install it. {google_jib}[JiB] allows you to build container images without having the client tooling to build containers. However, Docker is required to build GraalVM native image using the Quarkus Native Builder image.
