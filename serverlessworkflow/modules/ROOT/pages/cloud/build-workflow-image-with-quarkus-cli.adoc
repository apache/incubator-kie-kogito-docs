...
On this guild you will learn how to build a Serverless Application using *Quarkus CLI*.

== Pre Requisites

Before starting make sure you have the needed dependencies installed.
For this tutorial we assume that you already have your Serverless Workflow ready to be built inside
a container image. If not, you can use some of the {quarkus-kogito-examples}[examples], for this tutorial we will be using the {example-project}[serverless-workflow-greeting-quarkus].
...
- GraalVM {graalvm_min_version}: Note that, GraalVM is option if you already have the expected version installed locally.


== Getting started

First, let's clone the {kogito-examples}[kogito-examples] repository and access the {example-project}[serverless-workflow-greeting-quarkus] example.
...
If you wish to run the example beforehand, follow the steps described on the example.
...
Quarkus provides a few different extensions to build Container Images, like `JiB`, `docker`, `s2i` and `buildpacks`.
For this guide we will use `JiB`. If you already use another extension or have curiosity to know how it works, please visit this {quarkus-container-images}[Quarkus' guide].

The Kogito Examples does not include the needed extensions, we'll need to add it using the Quarkus CLI:
...
If you inspect the pom.xml, you can check the new dependency added:
