...
This document describes how to build Serverless Application Container Image using the link:{quarkus-cli}[Quarkus CLI].

.Prerequisites
* Ensure that the required dependencies are installed.
* Your Serverless Workflow application is ready in a container image. For more information about creating your Serverless Workflow application, see link:getting-started/create-your-first-workflow-service.adoc[Creating your first Serverless Workflow service].
* link:{quarkus_cli}[Quarkus CLI] {quarkus_version} is installed.
...
Quarkus provides a few extensions to build container images, such as `Jib`, `docker`, `s2i`, and `buildpacks`. For more information about the Quarkus extensions, see link:{quarkus_container_images_url}[Quarkus documentation].
