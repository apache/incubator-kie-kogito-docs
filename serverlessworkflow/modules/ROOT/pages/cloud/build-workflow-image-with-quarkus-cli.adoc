...
To get started with building Serverless Workflow images, you can use the link:{kogito_sw_examples_url}/serverless-workflow-greeting-quarkus[serverless-workflow-greeting-quarkus] example application.
...
.Procedure
. Clone the link:{kogito_examples_url}[kogito-examples] repository and navigate to the link:{kogito_sw_examples_url}/serverless-workflow-greeting-quarkus[serverless-workflow-greeting-quarkus] example application.
+
.Clone an example application
[source,shell,subs="attributes+"]
----
git clone --branch main {kogito_examples_url} && cd kogito-examples/serverless-workflow-examples/serverless-workflow-greeting-quarkus
----

. To run the example application, follow the instructions in xref:getting-started/create-your-first-workflow-service.adoc[Creating your first Serverless Workflow service].
...
[NOTE]
====
The added Jib extension caches the `target/lib` directory. Based on the size of your project, Jib speeds up the rebuilds. If you want to push your Serverless Workflow application to a registry, ensure that you define the following properties as required:
...
.Prerequisites
* You have created a Quarkus project.
* Quarkus CLI is installed. For more information about installing the Quarkus CLI, see link:{quarkus_cli_url}[Installing the Quarkus CLI].
...
[IMPORTANT]
====
When you build Container Images using a remote Docker Daemon, such as Minikube, then use the following system property instead of `-Dquarkus.native.container-build=true` property:
...
After building your Serverless Workflow image using Quarkus CLI, you can test the built image.
...
. To invoke the Serverless Workflow application, execute the following command once the container is started:
+
--
[tabs]
====
