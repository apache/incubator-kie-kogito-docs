...
You can also set the `imagePullPolicy` to `Never` or `IfNotPresent`, otherwise Minikube pulls the images from a remote registry.
...
The previous command starts in a loop. Therefore, this command must be running through the deployment process. On Mac and Windows, you might be required to provide the user password.
...
You can find the following generated files in `target/kubernetes` directory:
...
The link:{quarkus_cli_url}[Quarkus CLI] can be used to abstract all of these complexity mentioned before and deploy it automatically after the build completes. To instruct Quarkus to perform the deployment automatically for you, you need to set a few system properties, which are:
...
----
<1> - The Knative resource name.
<2> - Whether or not install deploy the on the active kubernetes cluster.
<3> - Force to not push images when using Minikube's remove Docker daemon to avoid image validation.

[IMPORTANT]
====
This functionality only works with Quarkus 2.10.2.Final or newer, for more information please visit this
