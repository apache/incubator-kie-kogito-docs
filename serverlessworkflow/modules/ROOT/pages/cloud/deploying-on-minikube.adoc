...
. Deploy your {context} application using the Minikube registry.
+
--
...
Once you have installed your {context} application into the Minikube registry, you can use the command-line tools, such as Knative CLI or `kubectl` to initiate the deployment process.

.Prerequisites
* {context} application is installed.
* Knative CLI is installed. +
For more information, see link:{kn_cli_url}[Install the Knative CLI].

.Procedure
. In a command terminal, execute the following command to deploy your {context} application using Knative CLI:
+
--
...
You can also use kubectl command-line interface and plain Kubernetes objects to deploy your {context} application. +
...
. To generate the `knative` `yaml|json` descriptors, set the following properties in the `application.properties` file of your {context} application:
+
--
