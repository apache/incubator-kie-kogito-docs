...
This document describes how to deploy a Serverless Application using a local Kubernetes cluster link:{minikube_url}}[Minikube] as well the steps required to get the Knative plataform running. Please make sure to read the Get Started Minikube guide and assure that your machine meets the System Requirement.
...
+
Knative CLI offers a plugin called `quickstart` that will configure everything that is needed for you, to install it follow this link:{kn_cli_quickstart_plugin_url}[steps].
+
...
----
+
NOTE: To install everything manually, there is this cool link:{knative_on_minikube_step_by_step_url}[post] from Red Hat Developers blog.
+
...
== Deploying a Serverless Application on Minikube

At this point, is expected that you:

* Followed the previous steps to install Minikube and Knative.
* Have the CLI tools properly installed.
* Have a Serverless Application Container already built.
...
For this tutorial we will use the Application used on the xref:cloud/build-workflow-image-with-quarkus-cli.adoc[Building Serverless Workflow Images using Quarkus CLI] guide.

=== Preparing the environment

Before start the procedure to install de Serverless Application, make sure you have started the `minikube tunnel` pointing to the `knative` profile.
...
----
+
NOTE: The tunnel command will start in a loop, it must be running during all this guide.
...
----
+
Now on, all the commands issued against Minikube, will use this namespace
+
[NOTE]
====
...
Let's proceed with the installation of our Serverless Application using the container generated in the xref:cloud/build-workflow-image-with-quarkus-cli.adoc[Building Serverless Workflow Images using Quarkus CLI] guide. For this example we will use the `native` image to take advantage of its faster startup. Below you can find two examples, one using the Knative CLI `kn` and another using the `kubectl` command line tool.
...
Instead creating the `knative` 'yaml|json' descriptors, you can leverage on the Quarkus Kubernetes extension and Kogito knative addon to generate that for you. To use this, make sure the following dependencies are present on the Application dependencies:
...
Set the following system properties so the files can be generated:
...
NOTE: There is a small bug on the 'Decorate API' where some system properties are not take effect, for more information please take a look at this link:{https://github.com/quarkusio/quarkus/issues/23832}[Quarkus issue].
