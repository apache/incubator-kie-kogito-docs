...
:kubectl_prereq: `kubectl` command-line tool is installed. Otherwise, Minikube handles it.
...
:registry: Minikube's
:container_push: Property to whether or not push images. When using Minikube's remove Docker daemon to avoid image validation
