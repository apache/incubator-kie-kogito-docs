...
Once you have pushed your workflow application into the {registry} registry, you can use the command-line tools, such as Knative CLI or `{command_line_tool}` to initiate the deployment process.
...
[NOTE]
====
Depending on the cluster type you have deployed the workflow application, the service URL might be different. Pay attention to the output to use the correct URL in the next topic.
====
