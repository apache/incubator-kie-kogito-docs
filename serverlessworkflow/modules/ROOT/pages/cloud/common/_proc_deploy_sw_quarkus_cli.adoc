...
----
--
. To deploy the workflow application using Quarkus CLI, set the following system properties in `application.properties`
file:
+
--
...
quarkus build -DskipTests \
  ...
  -Dquarkus.container-image.registry={k8s_registry} \
  -Dquarkus.container-image.group={default_namespace} \
  -Dquarkus.container-image.tag=1.0-SNAPSHOT
----

NOTE: The activated maven profile is named as a `container`, which provides the needed system properties to build the target container image.
