...
* `{command_line_tool}` command-line tool is installed.
...
+
--
To use the Quarkus Kubernetes extension and Kogito Knative add-on, ensure that the following dependencies are available
in the `pom.xml` file and Gradle:
...
[IMPORTANT]
====
The image used in this section was already built in the previous topic, if you do not have done it yet, you can take a look at xref:cloud/quarkus/build-workflow-image-with-quarkus-cli.adoc[Build Workflow Image with Quarkus CLI].
take a look xref:cloud/quarkus/build-workflow-image-with-quarkus-cli.adoc[Build Workflow Image with Quarkus CLI].
====
