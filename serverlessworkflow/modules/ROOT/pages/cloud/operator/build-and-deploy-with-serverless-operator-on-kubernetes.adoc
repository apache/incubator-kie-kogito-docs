= Building and deploying with Serverless Operator on Kubernetes 
:compat-mode!:
// Metadata:
...
If you already have a container built and pushed to a container registry and you want to deploy it on the Kubernetes cluster, then you can do it without the operator following the guide xref:cloud/quarkus/deploying-on-kubernetes.adoc[Deploying your {context} application on Kubernetes].
