...
Every time you need to change the workflow definition the system will (re)build a new immutable version of the application. If you're still in development phase, please see the xref:cloud/operator/developing-workflows.adoc[] guide.

Follow the <<building-kubernetes, Kubernetes>> or <<building-openshift, OpenShift>> sections of this document based on the cluster you wish to build your workflows.
...
The operator can build workflows on Kubernetes or OpenShift. On Kubernetes, it uses link:{kaniko_url}[Kaniko] and on OpenShift a link:{openshift_build_url}[standard BuildConfig]. This build system is not tailored for advanced use cases and you can do only a few customizations.
...
[TIP]
====
If you have deployed a workflow for xref:cloud/operator/developing-workflows.adoc[development] you already have a `SonataFlowPlatform` custom resource. Just edit the one you have.
====

Following a very basic `SonataFlowPlatform` Custom Resource example:

.Example of a basic SonataFlowPlatform
[source,yaml,subs="attributes+"]
----
apiVersion: sonataflow.org/v1alpha08
kind: SonataFlowPlatform
...
You don't need to do anything to build on OpenShift since the operator will configure everything for you using the defaults. There are a few customizations you can do described in the <<configure-build-system>> section.

In general, the operator will create a backed `BuildConfig` and use the internal OCI to build the workflow application using the mapped xref:cloud/operator/referencing-resource-files.adoc[resource files] and your workflow definition. After the build is finished, the image will be pushed to the internal OpenShift registry backed by an `ImageStream` object.
...
# On OpenShift
# TODO: figure out the full command
GREETING_SVC=$(oc get route/greeting -n workflows )
...
----
{"id":"b5fbfaa3-b125-4e6c-9311-fe5a3577efdd","workflowdata":{"name":"John","languag#e":"English","greeting":"Hello from JSON Workflow, "}}
----
