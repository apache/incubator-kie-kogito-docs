...
In general, the operator will create a link:{openshift_build_url}[`BuildConfig` to build] the workflow application using the mapped xref:cloud/operator/referencing-resource-files.adoc[resource files] and your workflow definition. After the build is finished, the image will be pushed to the internal OpenShift registry backed by an `ImageStream` object.
...
1. Expose the workflow application so you can access it:
+
...
GREETING_SVC=$(oc expose svc greeting -n workflows --output='jsonpath={.status.ingress[0].host}')
----
+
