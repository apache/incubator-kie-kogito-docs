...
To get started, you can use an editor of your choice to create a new Kubernetes Workflow YAML definition. For example:
...
<1> The annotation `sw.kogito.kie.org/profile: dev` tells the operator to deploy your workflow using the devlopment profile. This means that the operator will build a running instance of the workflow ready to receive changes during your development cycle.
...
You can make changes to the workflow YAML using any Kubernetes editor such as `kubectl`. The operator ensures that the latest workflow definition is running and ready. 
...
To ensure the workflow is running in a healthy state, the operator deploy the application pod with health checks probes.
...
The {operator_name} is under active development. Sometimes a feature might not be available yet. Please see xref:cloud/operator/known-issues.adoc[] for a compreehensive list of available features.
...
See xref:cloud/operator/configure-workflows.adoc[] for more information.
...
If you found an issue that a cause is not listed in this section, please link:{docs_issues_url}[let's know].
