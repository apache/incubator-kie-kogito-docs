...
|`/deployments`
|Application deployment directory containing Quarkus application components. Must be accessible and have write access for the operator's user.
...
|`/deployments/config`
|Workflow Application properties. This path is used by the operator to mount managed and user properties, which must be accessible, and the application should be able to read them at runtime. The image shouldn't create this path upfront; it will be created by the Kubernetes config map controller.
