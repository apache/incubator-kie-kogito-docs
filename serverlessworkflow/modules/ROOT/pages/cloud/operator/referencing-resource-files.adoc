...
The `Hello Service` workflow int the example offer two options. You can either create two `ConfigMaps`, each for one file, to have a clear separation of concerns or group them into one.
...
To make it simple, you can create only one `ConfigMap`. Navigate into the directory where your resources files are available and create the config map using following command:
