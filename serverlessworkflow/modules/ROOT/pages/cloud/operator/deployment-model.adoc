...
To change between profiles, you must set the SonataFlow custom resource annotation `org.sonataflow/profile`:
...
metadata:
    annotations:
        org.sonataflow/profile: gitops <1>
----

<1> Example configuration that sets the `gitops` profile. Valid values are: `dev`, `gitops`, or `preview`.
