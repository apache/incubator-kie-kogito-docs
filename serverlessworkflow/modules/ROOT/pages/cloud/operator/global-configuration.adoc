...
For example, suppose you change the `sonataFlowDevModeImageTag` property, given that you already have a workflow deployed in _devmode_. In that case, the operator won't roll out a new one with the new image configuration. Only new deployments will be affected.
