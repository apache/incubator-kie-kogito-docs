...
You will use SonataFlowPlatform CR's spec->persistence->postgresql to specify database to be used by Data Index and Jobs Service. And specify the database migration mode to use under spec->services->dataIndex or jobService->persistence->dbMigrationStrategy with value such as job, service or none.
