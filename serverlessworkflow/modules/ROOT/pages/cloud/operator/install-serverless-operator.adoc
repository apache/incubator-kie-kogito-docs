...
To install the operator on OpenShift follow the document "link:{openshift_operator_install_url}[Adding Operators to a cluster]" from the OpenShift's documentation.
...
To remove the operator on OpenShift follow the document "link:{openshift_operator_uninstall_url}[Deleting Operators from a cluster]" from the OpenShift's documentation.

== {product_name} Operator Kubernetes Installation

To install the operator on Kubernetes follow the document "link:{kubernetes_operator_install_url}[How to install an Operator from OperatorHub.io]" from the OperatorHub's documentation.
...
[WARNING]
====
If you're running on Kubernetes or OpenShift, It's highly recommended to install the operator from the OperatorHub or OpenShift Console instead since the installation is managed by OLM. Use this method only if you need a snapshot version or you're running locally on Minikube or KIND.
====
