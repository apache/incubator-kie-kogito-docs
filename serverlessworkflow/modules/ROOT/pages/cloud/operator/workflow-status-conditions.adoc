...
When building an image for a Workflow resource, a `KogitoServerlessBuild` to will first be created and the operator will wait for the associated build to finish. Once the build is complete, the Workflow transitions to the deployment phase, which will provision a new Workflow service Pod to run with the built image.
