...
Note that, the `kogito-examples` already have this extension added by default, can be activated with the maven profile
called `container`. The steps to add the extension in your Serverless Workflow application are:
[source,shell]
----
...
* `quarkus.container-image.registry`: To define the registry address of the image, such as `quay.io`. If using OpenShift
