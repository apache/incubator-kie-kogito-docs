...
If You already have a OpenShift cluster available you can skip this section.
But, if you don't have or want to have it locally, please proceed further with this link:{ocp_local_url_install}[installation steps].
...
When everything is ok, we can go ahead and make sure that the Knative Serving is configured, for this example only it
...
If you got 'False' as result for the items above, it means that `Serving` is not fully installed, to install, deploy
