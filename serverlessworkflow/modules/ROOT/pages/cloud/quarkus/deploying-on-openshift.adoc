...
[NOTE]
====
You can use the link:build-workflow-image-with-quarkus-cli.html#proc-building-serverless-workflow-application-using-native-image[native image] due to the faster startup.
====
...
This command should assign to the `OPENSHIFT_REGISTRY_URL` variable the publicly accessible URL for the registry. If it does not return it, it does mean that it is not exposed, to expose the registry follow link:{ocp_expose_registry_url}[these] steps.
it does mean that it is not exposed, to expose the registry follow link:{ocp_expose_registry_url}[these] steps.

The OpenShift's registry requires you to be logged in to be allowed to push Container Images. If you are already logged in the cluster you can use your token to authenticate in the OpenShift's registry. To log in, save the URL from the command above:
...
You can read the further the next sections which explain different approaches deploy your {product_name} application, as such:
