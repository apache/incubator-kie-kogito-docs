...
Before proceeding further, make sure that you have access to the OpenShift cluster, the OpenShift Serverless operator properly installed and the `Knative Serving` is ready for use. For more information on each topic, please refer the following guides:
...
If no information or a error is returned, it does mean that it is not exposed, to expose the registry follow link:{ocp_expose_registry_url}[these] steps.
