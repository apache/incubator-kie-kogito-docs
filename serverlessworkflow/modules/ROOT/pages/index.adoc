...
--
[.card-title]
xref:cloud/build-and-deploy-with-serverless-operator-on-kubernetes.adoc[Build and deploy in Kubernetes a {context} application using the {product_name} {context} Operator]
[.card-description]
Learn how to build and deploy using the Kogito Serverless Workflow Operator a serverless workflow application on Kubernetes
--
