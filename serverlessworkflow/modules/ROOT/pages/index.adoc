...
{product_name} is a tool to build cloud-native workflow applications. {product_name} can be used to do services and events orchestration and choreografy. Currently, with {product_name} you can integrate with services and events in your architecture using:

1. **CloudEvents**. Ideal for an Event-Driven architecture where the services are ready to consume and produce events working in more reactive way.
...
3. **Internal Service execution or invocation**. {product_name} is also a framework to build Java Workflow applications. You can use it to create custom Java services to run an application process within the same instance.

Learn how to do start creating, managing, and deploying your workflow applications with the guides below.
