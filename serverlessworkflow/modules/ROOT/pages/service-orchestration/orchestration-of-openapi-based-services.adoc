...
The examples in this document assumes that you have following prerequisites:
...
* You have created a Serverless Workflow project.
+
...
Kogito follows the link:{spec_doc_url}#using-functions-for-restful-service-invocations[CNCF Serverless Workflow specification function definition] to send requests to remote REST services using an OpenAPI specification reference as shown in the following example:
...
[NOTE]
====
In the previous example function definition, the `type` attribute can be ommited as the link:{spec_doc_url}#Function-Definition[default value] is `rest`.
====
...
[IMPORTANT]
====
The OpenAPI specification files must be available during the build time, because Kogito leverages the internal code generation feature to send requests on runtime. Therefore, when you build your application image, the engine will not have the access to the files. 

For more information about building Serverless Workflow images, see {build-workflow-image-with-quarkus-cli}[Building Serverless Workflow Images using Quarkus CLI].
====
...
In case, the OpenAPI service that you want to add to the Serverless Workflow does not contain a specification file, you can either create a specification file or update the OpenAPI service to generate and expose the file.
...
== Sending requests with the REST calls based on the OpenAPI specification

To send requests with the REST calls that are based on the OpenAPI specification files, you need to perform the following procedures:

. Define the function references
. Access the defined functions in the Serverless Workflow states

[[proc-define-openapi-functions]]
=== Defining OpenAPI functions in Serverless Workflow
...
+
Once you have access to the OpenAPI specification files, you can define the functions in the Serverless Workflow. 
...
.Procedure
. Copy the OpenAPI specification files of the service(s) that you want to invoke into your Serverless Workflow service directory that is `src/main/resources/specs`.
+
--
...
=== Accessing functions in the Serverless Workflow states

After defining the function definitions, you can access the defined functions in the Serverless Workflow states.

.Prerequisites
* You have defined the OpenAPI functions in Serverless Workflow. For more information, see <<proc-define-openapi-functions, Defining OpenAPI functions in Serverless Workflow>>.
...
. To map the arguments of a function, you can refer to the parameters that are described link:{open_api_spec_url}#operation-object[Operation Object] section of OpenAPI specification.
+
--
...
<3> Arguments definition. In the previous example, Kogito adds the argument `petId` to the request path before sending a request.
--

[[proc-configuring-openapi-service-endpoint-url]]
== Configuring the endpoint URL of OpenAPI services

After accessing the function definitions in Serverless Workflow states, you can configure the endpoint URL of OpenAPI services. 

Kogito uses the link:{quarkus_openapi_gen_url}[Quarkus OpenAPI Generator extension] and the link:{quarkus_rest_client_url}[Quarkus REST Client], and all the configurations are based on these components, which are backed using {quarkus_config_url}[Quarkus Configuration].

.Prerequisites
* You have defined the function definitions in the Serverless Workflow. For more information, see <<proc-define-openapi-functions, Defining OpenAPI functions in Serverless Workflow>>.
* You have the access to the defined functions in the Serverless Workflow states. For more information, see <<proc-access-function-sw-states, Accessing functions in the Serverless Workflow states>>.
...
For more information about how Kogito sanitizes file names, see link:{mp_config_env_vars_url}[Environment Variables Mapping Rules].
--
...
. To avoid hardcoding the URL in the application properties, you can use link:{quarkus_config_url}#with-environment-variables[environment variables substitution], as shown in the following example:
+
--
...
For more information about endpoints and general configuration, see {configuring-openapi-services-endpoints}[Configuring OpenAPI services endpoints].

[NOTE]
====
In a future release, Kogito will provide support for Kubernetes Service Discovery feature. Using the Kubernetes Service Discovery feature, you can point to a Kubernetes or Knative object that you want to call. After that, the engine will automatically configure the endpoints once you deploy the endpoints in the cluster.
====
--
...
* {camel-k-integration}[Integrating Camel K]
* {authention-support-for-openapi-services}[Authentication for OpenAPI services in Serverless Workflow]
