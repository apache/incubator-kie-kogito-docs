...
The examples in this document assume that you have the Kogito tooling installed and have created a Serverless Workflow project. 
...
[NOTE]
====
The function `type` attribute can be ommited since the link:{spec_doc_url}#Function-Definition[default value] is `rest`.
====

The `operation` attribute is a string composed by:

1. The URI that the engine uses to locate the specification file
2. The operation identifier (`operationId`), which you find in the link:{open_api_spec_url}#fixed-fields-7[OpenAPI specification file].
...
Once you have access to the OpenAPI specification files, you can create the function definitions in the workflow. You can use the link:{kogito_sw_examples_url}/serverless-workflow-temperature-conversion[serverless-workflow-temperature-conversion] example application as a reference.
