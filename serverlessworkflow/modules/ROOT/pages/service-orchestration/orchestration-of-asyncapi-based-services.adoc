...
[NOTE]
====
Remember that, as happens with `REST` and `RPC` types, the URI part of `operation` property supports `http`, `https` and `file` schemas. If no schema is present, it assumes the specification file is on project classpath.
====
