...
The `gRPC` functionality is included within Kogito Serverless Workflow dependencies, which means you do not need to add specific dependencies for `gRPC` to work with Serverless Workflow.
...
Therefore, since in this example a service called `Greeter` is invoked, the snippet below is added to the properties. 
