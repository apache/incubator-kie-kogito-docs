...
grpc link:{grpc_enum_url}[specification] requires enumeration types to have a default value. Default value is not included in the server response payload. Therefore it is recommended to use as default a meaningless value like UNKNOWN. 
If, for any reason, your default value is semantically valid and you want it to be included in the Workflow model, you must set `kogito.grpc.enum.includeDefault` to true. Doing so, enumeration fields are always filled by the Workflow if the server response does not include them. 
