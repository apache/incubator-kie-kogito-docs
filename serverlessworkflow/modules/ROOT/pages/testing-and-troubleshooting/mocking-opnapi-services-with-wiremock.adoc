...
* A Serverless Worlfow project is created and orchestrates the OpenAPI services.
+
For more information about creating a Serverless Workflow project and orchestrating OpenAPI services, see link:{create-your-first-workflow-service}[Creating your first Serverless Workflow service] and link:{orchestration-of-openapi-based-services}[Orchestrating the OpenAPI services].
...
+
For more information about creating a workflow project, see link:{create-your-first-workflow-service}[Creating your first Serverless Workflow service].
...
. Define the method that you selected to start the WireMock server as shown in the following example:
+
--
...
----
<1> Creates the configuration for the WireMock server. You must use a dynamic port when possible since it prevents the tests from failing due to port conflicts. When you cannot use a dynamic port, you can use the `WireMockConfiguration#port(int)` method to use a fixed port number.
...
You can also start the WireMock server to be used in a specific server.
