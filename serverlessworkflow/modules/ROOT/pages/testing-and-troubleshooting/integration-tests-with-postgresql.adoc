...
:availableTestcontainers: https://github.com/kiegroup/kogito-runtimes/tree/main/kogito-test-utils/src/main/java/org/kie/kogito/testcontainers
:quarkusTestResourceLifecycleManager_class: https://github.com/quarkusio/quarkus/blob/main/test-framework/common/src/main/java/io/quarkus/test/common/QuarkusTestResourceLifecycleManager.java
...
In order to simulate that scenario and verify the service behavior, we are using different Quarkus annotations and tools
 that simplify this work like:

* *@QuarkusIntegrationTest* used to launch and test the artifact produced by the Serverless Workflow Quarkus build. For more
details see {quarkus_testing_guide_integration_test}[@QuarkusIntegrationTest] guide.
* *@QuarkusTestResource* allows you to define test resources that need to be started before running the test. For more details see
...
You can access this example application in {kogito_sw_examples_url}[Kogito Examples] GitHub repository.

[[creating-integration-test]]
== Creating the integration test with PostgreSQL
Let's create the integration test for a specific Kogito Serverless Workflow with PostgresQL:

First, first make sure that the Application's 'pom.xml' have the `kogito-quarkus-test-utils` dependency.
--
...
----
--
This allows us to use predefined test resources such as: **PostgreSqlQuarkusTestResource**

[NOTE]
====
This test framework allows the usage of predefined test resources,  you can see {availableTestcontainers}[here] that can be used to test more complex scenarios.
====
...
<2> Adds the reference to the predefined PostgreSqlQuarkusTestResource in kogito-quarkus-test-utils. The annotation
...
Once you specify the required resources and annotations, you can start testing the different interactions with the application as the guide link:{basic-integration-test-with-restassured}[Testing your Serverless Workflow application using REST Assured] describes.
...
PostgreSqlQuarkusTestResource is a predefined test resource provided by kogito-quarkus-test-utils that starts a container with the PostgreSQL instance.
That container has a full set of default configuration that are included in the integration test automatically

[NOTE]
====
As a test resource, it will run its {quarkusTestResourceLifecycleManager_class}[QuarkusTestResourceLifecycleManager] before the service test execution. More related info in {quarkus_testing_guide_test_resource}[Starting services before the Quarkus application starts].
====

The container has a full set of default configuration, such as PostgreSQL docker image name, default user, default password,...
All the needed configuration values to link the Kogito Serverless Workflow service with that instance, are added automatically to the integration test.
...
This allows you to reuse the same integration test with PostgreSQL enabled just for profiles with persistence enabled.
...
In the example, the `persistence` profile incorporates this property setting to execute the integration test with the ConditionalPostgreSqlQuarkusTestResource enabled.
...
Or in case of the Conditional Quarkus test resource, the `persistence` profile includes  `<enable.resource.postgresql>true</enable.resource.postgresql>`
.Command to run a test with a specific profile
[source,shell]
----
...
The testing procedure in this document is based on the `serverless-workflow-examples/serverless-workflow-newsletter-subscription` example application.
You can access this example application in {kogito_sw_examples_url}[Kogito Examples] GitHub repository
