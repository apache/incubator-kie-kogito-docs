...
:knative_eventing_components: https://knative.dev/docs/getting-started/first-broker/
...
:recap_of_http_based_testing_in_jvm_mode: {quarkus_testing_guide_url}#recap-of-http-based-testing-in-jvm-mode
...
The testing case followed in this document is based on the {kogito_sw_order_processing_example}[`serverless-workflow-order-processing`] example application.

[[sinkbinding-test-overview]]
== Overview of HTTP CloudEvents, SinkBinding and testing
--
The application we want to test must be configured to use {knative_eventing}[*Knative Eventing*], using standard HTTP POST requests to send and receive events between event producers and {knative_sink_binding_overview}[*sinks*].
...
This guide covers the testing of the Knative Service configured as {knative_sink_binding_overview}[*sink*].
...
* Configure the workflow application to use HTTP CloudEvents with {knative_sink_binding_impl}[*SinkBinding*].
Follow {consume-produce-events-with-knative-eventing_guide}[Consume and produce events with Knative eventing guide]
to enable {event_driven_architecture}[event-driven architecture] in the application using {knative_eventing}[*Knative Eventing*].

=== Adding test dependencies
Add the needed test dependencies to Serverless Workflow's application `pom.xml`.

** HTTP based Testing in JVM mode dependencies: *quarkus-junit5* and *rest-assured*. More detailed in {recap_of_http_based_testing_in_jvm_mode}[Testing your application Quarkus guide].
** {wiremock}[WireMock] framework: *wiremock-jre8*. Allowing to mock the server that plays as the sink.
** {awaitility}[Awaitility]: *awaitility* used to express expectations of an asynchronous system.
...
<12> {wiremock_verifying}[`verify`] if the request has hit the mock API with the expected event content.
...
The test {kogito_sw_order_processing_example_test_application_properties}[`application.properties`] file must contain the reference to the WireMockServer created.

.Test {kogito_sw_order_processing_example_test_application_properties}[`application.properties`] sink connection property
[source,properties]
----
...
The {kogito_sw_order_processing_example}[example] is composed by three workflows:
...
{awaitility_class}[`await()`] Allows the test to retry the validations until assert the verifications, or until the specified time expires (in this case after 60 seconds).
...
* {knative_eventing_components}[Knative Eventing components interaction: Source, Trigger, Broker, and Sink]
