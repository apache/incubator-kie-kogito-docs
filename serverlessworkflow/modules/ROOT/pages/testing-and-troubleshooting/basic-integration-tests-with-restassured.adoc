...
This document describes how to test your Serverless Workflow application using REST Assured. 
...
The testing procedure in this document is based on the `serverless-workflow-examples/serverless-workflow-testing-with-rest-assured` example application. You can access this example application in link:https://github.com/kiegroup/kogito-examples.git[Kogito Examples] GitHub repository 

The following procedure describes how to test a Serverless Workflow application that exposes the `hello` endpoint, which expects a `name` as a request parameter and returns `greeting` value:
...
.Prerequites
* You have cloned the `serverless-workflow-testing-with-rest-assured` example application from the link:https://github.com/kiegroup/kogito-examples.git[GitHub repository].
* The `pom.xml` file contains the required dependencies:
+
--
...
[INFO] Building jar: /home/helber/dev/kiegroup/kogito-examples/serverless-workflow-examples/serverless-workflow-testing-with-rest-assured/target/serverless-workflow-testing-with-rest-assured-1.0.jar
[INFO]
...
* xref:testing-and-troubleshooting/mocking-http-cloudevents-with-wiremock.adoc[Mocking HTTP CloudEvents using WireMock]
* xref:testing-and-troubleshooting/integration-tests-with-postgresql.adoc[Testing your Serverless Workflow application using PostgreSQL]
