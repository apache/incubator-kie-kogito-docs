...
By default, the persistence addon creates the required database schema structure. That is usually only recommended for easy of development and can be turned off by setting `kogito.persistence.auto.ddl=false`.
DDL scripts are available in case there is need to manually create the required database schema. These scripts are included in the https://repository.jboss.org/org/kie/kogito/kogito-ddl/{version}/kogito-ddl-{version}-db-scripts.zip[kogito-ddl-{version}-db-scripts.zip] artifact.
...
An alternative to this strategy is to set `kogito.workflow.version-strategy=project` in your application.properties file. This will make the engine to consider the Maven or Gradle project version as the actual version of all workflows in the project. For instance, by releasing a new verion of you Maven project, it will automatically update the workflow's version contained in the project.
