...
{context} generates metrics that can be consumed by Prometheus and visualized by dashboard tools, such as link:{openshift_micrometer_url}[OpenShift], link:{dashbuilder_url}[Dashbuilder], and link:{grafana_url}[Grafana].

This document describes how you can enable and expose the generated metrics to Prometheus.

[[proc-enable-metrics-sw]]
== Enabling metrics in {context}
...
== Metrics consumption in {context}

After enabling the metrics in {context}, the generated metrics can be consumed from OpenShift, Kubernetes, and Prometheus to visualize on different dashboard tools.
...
.Prerequisites
* Metrics is enabled in {context}.
+
For more information, see <<proc-enable-metrics-sw, Enabling metrics in {context}>>.
...
== Example metrics in {context}

In {context}, you can check the following example metrics:
...
[NOTE]
====
Internally, {product_name} uses Quarkus Micrometer extension, which also exposes built-in metrics. You can disable the Micrometer metrics in {context}. For more information, see link:{quarkus_micrometer_url}[Quarkus - Micrometer Metrics].
====
