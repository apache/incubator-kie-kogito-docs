...
----
{
  ...
  "metadata": {
    ...
    "kind": "Service",
    "name": "custom-function-knative-service", <4>
    "uri": "/function" <5>
  }
}
----
...
<3> Indicates that your custom function is of type `knative`
<4> The name of the service you want to invoke
<5> The resource path you want to access
...
. Deploy your workflow service to Knative. For more information on how to deploy a {product_name} {product_name} project to Knative, see the xref:cloud/quarkus/deploying-on-kubernetes.adoc[Deploying on Kubernetes].
