...
Here you can see in example, how the {data_index_ref} resource definition as part of a docker-compose definition
...
<3> When `KOGITO_DATA_INDEX_QUARKUS_PROFILE=kafka-events-support` or is not present, the {data_index_ref} is configured to use Kafka eventing.
<4> To initialize the database schema at start using flyway.


When kafka eventing is used, workflow applications need to be configured to send the events to the kafka topic `kogito-processinstances-events` allowing {data_index_ref} service to consume that, adding to application.properties:
[source,properties]
----
...
This deployment definition resource shows how the {data_index_ref} service configured and deployed to connect with an existing PostgreSQL database and to consume Knative events.

.Example `DataIndex` resource with PostgreSQL persistence and Knative eventing in a minikube environment :
[source,yaml]
----
apiVersion: v1
kind: Service
...
<1> Reference the right {data_index_ref} image to match with the type of Database, in this case `quay.io/kiegroup/kogito-data-index-postgresql:latest`
<2> Provide the database connection properties
<3> KOGITO_DATA_INDEX_QUARKUS_PROFILE: http-events-support to use the http-connector.
<4> To initialize the database schema at start using flyway
...
When this {data_index_ref} service is deployed, a link:{kubernetes_configmap_url}[`configMap`] resource is created for the `application.properties` configuration of the {data_index_ref} service.
