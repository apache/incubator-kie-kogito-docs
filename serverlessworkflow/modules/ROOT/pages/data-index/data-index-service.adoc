...
[NOTE]
====
Usually, when using docker-compose the workflow application generates the container image that is added to docker-compose. If the Kafka eventing configuration values weren't there before the container image creation, they need to be passed as image extra environment variables.
...
[NOTE]
====
If that configuration values weren't there before the container image creation, they need to be passed as image extra environment variables. More details about customizing Quarkus generated images can be found in {quarkus_container_image_customizing_url}[Quarkus Container Images Customizing] guide.
====
