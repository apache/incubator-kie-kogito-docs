...
This document describes how you add the {data_index_ref} features to your runtime service. You simply need to add the {data_index_ref} extension to the runtime and
the workflow related data will be collected and stored in the database, enabling the GraphQL endpoint to execute queries and perform management operations over the {workflow_instances}.
...
The {data_index_ref} service has been designed to store and manage data from different workflows that can be hosted in different runtimes.
...
Once one of these `kogito-addons-quarkus-data-index` addons is added to a Workflow application, that service incorporates the functionality to index and store the workflow data and also incorporates the GraphQL endpoint to perform queries and management operations.

In the same way as the {data_index_ref} service, there is a specific addon for each type of persistence you want to work with. Currently, we can find {data_index_ref} addons for: link:{postgresql_url}[PostgreSQL]
link:{infinispan_url}[Infinispan] and  link:{mongo_url}[MongoDB]

[IMPORTANT]
====
The {data_index_ref} addon distribution added to the application must match the application's kind of persistence, regarding the indexed data, which can be stored in the same database.
The addon will share the data source used by the application where it is added, and it will create separate tables for that purpose.
====

When any of the {data_index_ref} addons is added:

* The communication with the workflow application is direct, the workflows data is *not* transmitted or consumed through events, they are stored directly in the configured database.
...
.Procedure
. Add the required {data_index_ref} addon dependencies to the `pom.xml` file of your workflow application:
+
--
...
+
--
Prepare the application to run in dev mode, avoid starting the {data_index_ref} Quarkus Dev service, and specify if the GraphQL UI needs to be available.
