...
**Example 1)** Successful execution when calculating the currency exchange from `EUR` to `USD`
...
**Example 2)** Unsupported currency error when calculating the currency exchange from `EUR` to `MXN`
...
**Example 3)** Unexpected errors when accessing `acme-financial-service`

To execute this case, first you must go the terminal window where you started the `acme-financial-service` and stop it with CTRL+C.
