...
This document describes the examples of how you can implement and configure a Serverless Workflow that orchestrates the interaction with an OAuth 2.0 secured REST service.

For more information about orchestrating and configuring OpenAPI services, see {orchestration-of-opnapi-based-services}[Orchestrating the OpenAPI services], {configuring-openapi-services-endpoints}[Configuring OpenAPI services endpoints in different environments], and {authentication-support-for-openapi-services}[Authentication for OpenAPI services in Serverless Workflow].

[[ref-example-oauth2-orchestration]]
== Example of OAuth 2.0 orchestration in Serverless Workflow

To understand the example of OAuth 2.0 orchestration in Serverless Workflow, you can use the link:{kogito_sw_examples_serverless_workflow_oauth2_orchestration_url}[`serverless-workflow-oauth2-orchestration-quarkus`] example application. This example application implements a workflow related to currency exchange calculations, which orchestrates an OAuth 2.0 secured REST service provided by Acme Financial Services.  
...
* Orchestration with services provided by Acme Financial and currency exchange calculations.
* Authentication requirements to access the services provided by Acme Financial.
...
+
--
The `currency-exchange-workflow` in `serverless-workflow-oauth2-orchestration-quarkus` example application is a Serverless Workflow that implements the currency exchange calculations.
...
The `validateInputs` function is used to execute the custom Java processing as part of the Serverless Workflow. For more information about custom functions, see link:{spec_doc_functions_for_custom_function_types_url}[Serverless Workflow specification].
...
To access the `acme-financial-service` REST service in `currency-exchange-workflow`, a Serverless Workflow function such as `getExchangeRate` is used. For more information about using functions for REST services, see link:{spec_doc_functions_for_restful_service_invocation_url}[Serverless Workflow specification].
...
To configure the link:{quarkus_guide_security_openid_connect_client_url}[Quarkus OpenId Connect Client (OIDC)] for `acme_financial_service_oauth` service, you must follow the rules described in link:{authentication-support-for-openapi-services-oauth-example}[Example of OAuth 2.0 authentication].
...
. In a separate command terminal window, navigate to the `currency-exchange-workflow` directory and start the Quarkus application of currecy exchange workflow:
+
--
...
+
--
.Example of successful execution when calculating the currency exchange from `EUR` to `USD`:
[tabs]
====
Example request::
+
...
.Example of unsuccessful execution when calculating the currency exchange from `EUR` to `MXN`
[tabs]
====
Example request::
+
