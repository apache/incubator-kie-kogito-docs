...
This document describes the supported authentication types and how to configure them to access the OpenAPI service operations that are used in Serverless Workflows.

For information about orchestrating and configuring the OpenAPI based services, you must see the following documents:

* link:{orchestration-of-opnapi-based-services}[Orchestrating OpenAPI based services]
* link:{configuring-openapi-services-endpoints}[Configuring OpenAPI services endpoints in different environments]

[NOTE]
====
The OpenAPI support for Serverless Workflow is based on the Quarkus OpenAPI generator extension. For more information about Quarkus OpenAPI generator, see link:https://github.com/quarkiverse/quarkus-openapi-generator[Quarkus - OpenAPI Generator].
====

[[con-openapi-services-authentication]]
== Overview of OpenAPI services authentication

According to the OpenAPI specification, you can secure an OpenAPI service operation by using a security scheme. These security schemes are defined in the link:https://spec.openapis.org/oas/v3.1.0#security-scheme-object[Security Scheme Object] definitions of the OpenAPI specification file.

The security scheme type defined for an OpenAPI service operation is used to identify the required configuration, when a Serverless Workflow invokes the same operation.
...
* `filename` is the name of the file containing the OpenAPI specification, such as `security_example.json`.
* `security_scheme_name` is the name of the security scheme object definition in the OpenAPI specification file, such as `http_basic_example` or `api_key_example`.
* `auth_property_name` is name of the property to configure, such as `username`. This property depends on the defined security scheme type.
...
For a complete example, see link:{orchestrating-third-party-services-with-oauth2}[Orchestrating third party services using OAuth2 authentication].
...
The API key scheme contains an additional `name` property, specifying where to add the API key in the request token, such as _header_, _cookie_, or _query_.

However, adding the API key is automatically managed without additional configurations. Following the previous example, the API key is passed as an `http` request parameter, such as `api-key-name`.
...
.Example `security-example.json` file for OAuth2 authentication
[source, json]
----
{
  "openapi": "3.1.0",
...
Unlike the `http basic`, `http bearer`, and `apiKey` security schemes, the Oauth2 authentication relies on the link:https://quarkus.io/guides/security-openid-connect-client[Quarkus OpenId Connect (OIDC) and Oauth2 Clients and Filters]. Therefore, you must add link:https://quarkus.io/guides/security-openid-connect-client#oidc-client-filter[Quarkus OIDC Client Filter Extension] to your project as shown in the following example:
...
You can use the authorization token propagation with OpenAPI operations that are secured using the `oauth2` or `http bearer` security scheme type. When the configuration is set, you can propagate the authorization tokens that are passed to your Serverless Workflow during the workflow creation. Along with the authorization token, you can pass the OpenAPI invocations that are executed by the same OpenAPI service.

The propagations must be configured individually for each security scheme, which enables you to configure the invocations properly that require the propagation.
...
[IMPORTANT]
====
The authotization tokens are propagated till the Serverless Workflow does not reach its waiting state. When the same Serverless Workflow is resumed, the tokens will not be propagated.
====

== Additional resources

* link:{orchestrating-third-party-services-with-oauth2}[Orchestrating third party services using OAuth2 authentication]
