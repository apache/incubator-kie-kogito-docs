...
provides a plug-in named `kn-plugin-workflow` for Knative CLI, which enables you to set up a local workflow project quickly using the command line.

This document describes how you can install and use the `kn-plugin-workflow` plug-in in 
ifeval::["{context}" == ""]
Serverless Workflow.
endif::[]
ifeval::["{context}" != ""]
