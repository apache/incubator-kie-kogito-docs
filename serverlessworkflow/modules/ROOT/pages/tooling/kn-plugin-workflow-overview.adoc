...
Serverless Workflow provides a plug-in named `kn-plugin-workflow` for Knative CLI, which enables you to set up a local Serverless Workflow project quickly using the command line.
...
You can use the Serverless Workflow plug-in to set up your local Serverless Workflow project quickly using Knative CLI. 
...
* Install `kn workflow` command as a plugin of the Knative CLI using the following steps:
...
[WARN]
==== 
Some systems might block the application to run due to Apple enforcing policies. To fix this problem, check the "Security & Privacy" section in the "System Preferences", "General" tab to approve the application to run. More information available at link:{apple_support_url}[this Apple support article].
====
.. Run the following command to verify that `kn-workflow` plug-in is installed successfully:
+
`kn plugin list`

After installing the Serverless Workflow plug-in, you can use `kn workflow` to run the related subcommands.
--
...
== Creating a Serverless Workflow project using Knative CLI

After installing the Serverless Workflow plug-in, you can use the `create` command with `kn workflow` to scaffold a new Serverless Workflow project in your current directory.

The `create` command sets up Quarkus project containing minimal extensions to build a Serverless Workflow project. Also, the generated Serverless Workflow project contains a "hello world" `workflow.sw.json` file in your `./<project-name>/src/main/resources` directory.

.Prerequisites
* Serverless Workflow plug-in for Knative CLI is installed.
+
For more information about installing the plug-in, see <<proc-install-sw-plugin-kn-cli, Installing the Serverless Workflow plug-in for Knative CLI>>.
...
== Building a Serverless Workflow project using Knative CLI

After creating your Serverless Workflow project, you can use the `build` command with `kn workflow` to build your Serverless Workflow project in your current directory and to generate a container image. 

The process of building your Serverless Workflow project produces a `knative.yml` file in the `./target/kubernetes` folder. If your workflow contains events, then the building process also generates a `kogito.yml` file.
...
* Serverless Workflow project is created.
+
For more information about created a workflow project, see <<proc-create-sw-project-kn-cli, Creating Serverless Workflow project using Knative CLI>>.

.Procedure
. In Knative CLI, enter the following command to build your Serverless Workflow project:
+
--
...
=== Strategy for building a Serverless Workflow project

You can use the following strategies to build a Serverless Workflow project and to generate the container image:
...
You can use the following commands to build a Serverless Workflow project and to generate a local image using Jib:
...
+
--
The process of building your Serverless Workflow project using Docker is straightforward and also a default approach. 
...
== Deploying a Serverless Workflow project using Knative CLI
...
* Serverless Workflow project is built.
+
For more information about building your workflow project, see <<proc-build-sw-project-kn-cli, Building Serverless Workflow project using Knative CLI>>.

.Procedure
. In Knative CLI, enter the following command to deploy your Serverless Workflow project:
+
--
...
[NOTE]
====
You can use the kubectl command line if you want to use a complex deployment setup for your Serverless Workflow project.
====
--

[[proc-build-sw-project-source]]
== Building a Serverless Workflow project from source

As an alternative, you can build your Serverless Workflow project from source.
...
. Navigate to the cloned `kie-tools` directory and enter the following command to access the Serverless Workflow plug-in:
+
--
.Access Serverless Workflow plug-in
[source,shell]
----
cd kie-tools
pnpm bootstrap
...
+
--
.Build Serverless Workflow project from source
[source,shell]
----
...
You have successfully built your Serverless Workflow project from source.
