...
The Start New Workflow page is used start a workflow instance through a cloud event or a custom form depending on the workflow configuration. To navigate to the *Start New Workflow* page, click on the play button in the *Actions* column of workflow definitions table.
...
If the workflow has a json schema configured in the `dataInputSchema` property, then a form is displayed in-order to start a new workflow instance. You can fill-in the required form details and click on the *Start* button to trigger the workflow.
