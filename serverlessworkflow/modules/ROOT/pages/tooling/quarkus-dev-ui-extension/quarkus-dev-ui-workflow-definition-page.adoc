...
The Start New Workflow page is used to start a workflow instance through a cloud event or a custom form, depending on the workflow configuration. To navigate to the *Start New Workflow* page, click on the play button in the *Actions* column of the workflow definitions table.

*If there is no json schema for the workflow*, then the workflow will be started by triggering a cloud event.
...
.Starting a workflow using cloud event
image::tooling/quarkus-dev-ui-extension/kogito-swf-tools-trigger-cloud-events.png[]

*If there is a json schema for the workflow* configured in the `dataInputSchema` property, then a form is displayed in-order to start a new workflow instance. You can fill-in the required form details and click on the *Start* button to trigger the workflow.
The *Reset* button is used to clear the form data.

.Starting a workflow using form
