...
.Procedure
. In the {serverless_logic_web_tools_name} web application, click on the *Cog wheel* (⚙️) on the top-right corner and go to the *Extended Services* tab.
...
+
A window appears containing your current operating system as selected and a link to download to the latest version of Extended Services.
...
.Prerequisites
* Extended Services tool installed and running.
...
. Click on the *Add connection* button and a window will be shown.
...
[NOTE]
====
If your OpenShift instance uses self-signed certificates, then you might need to enable `InsecureSkipVerify` on Extended Services.
====
