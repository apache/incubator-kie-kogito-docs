...
. If your OpenShift instance uses Self-Signed certificates, then you must also check the Insecurely disable TLS certificate validation option.
. Click *Connect*.
+
...
[NOTE]
====
If your OpenShift instance uses self-signed certificates, then you must enable the `InsecureSkipVerify`.
====
